...
const calculate = (total) => {
  ...
  if (calcList.length === 1) {
    return parseFloat(calcList[0]);
  }

  let final = operate(
    calcList[1],
    parseFloat(calcList[0]),
    parseFloat(calcList[2])
  );
...
operators.forEach((operator) =>
  operator.addEventListener("click", handleOperations)
);
operators.forEach((operation) => (operation.disabled = true));

const numbers = document.querySelectorAll(".number");
numbers.forEach((operator) =>
  operator.addEventListener("click", handleNumbers)
);
